...
if not st.session_state.authenticated:
    pw = st.text_input("Enter your super-ultra secret password (v24/06/2025 10:012h)", type="password")
    if pw == PASSWORD:
...
if upload_type == "Video":
    ...
    if video_file:
        if have_cv2:
            ...
            visual_analysis = False
            st.warning("Frame analysis disabled: install the 'opencv-python-headless' package to enable this feature.")
...
if st.button("✍️ Create article"):
    try:
        # 1. Transcripción y análisis visual
        if upload_type == "Video":
            transcription = ""
            visual_context = ""
            # Extraer frames si aplica
            ...
            with st.spinner("⏳ Transcribing audio with Whisper..."):
                with open(tmp_path, "rb") as audio_f:
                    tr = client.audio.transcriptions.create(model="whisper-1", file=audio_f, response_format="json")
        ...
        if upload_type == "Video" and not is_smn_video:
            ...
            full_prompt += f"\nSource network: {network}" + f"\nOriginal account: {username}" + f"\nOriginal URL: {original_url}"
